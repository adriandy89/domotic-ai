...
export class NotificationController {
  ...
  async handleSensorNotification(@Payload() payload: IUserSensorNotification) {
    ...
    const { user, deviceName, homeName, attributeKey, sensorKey, sensorValue } =
      payload;
    ...
    for (const channel of user.channels) {
      switch (channel) {
        ...
        case NotificationChannel.SMS:
          this.logger.log(
            `SMS notification not implemented. User: ${user.id}, Device: ${deviceName}, Attribute: ${attributeKey}`,
          );
          break;

        case NotificationChannel.PUSH:
          this.logger.log(
            `PUSH notification not implemented. User: ${user.id}, Device: ${deviceName}, Attribute: ${attributeKey}`,
          );
          break;

        case NotificationChannel.WEBHOOK:
          this.logger.log(
            `WEBHOOK notification not implemented. User: ${user.id}, Device: ${deviceName}, Attribute: ${attributeKey}`,
          );
          break;
  ...
  async handleRuleTelegramNotification(
    ...
    this.logger.log(
      `Received telegram notification for rule ${payload.ruleId}`,
    );
...
`;
      ...
      if (success) {
        this.logger.log(
          `Telegram notification sent successfully for rule ${payload.ruleId}`,
        );
      } else {
        this.logger.warn(
...
`;
    ...
    if (success) {
      ...
      this.logger.warn(
        `Failed to send telegram sensor notification to ${chatId}`,
      );
    }
  }
