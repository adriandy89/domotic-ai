...
export const LinkHomeUsers: React.FC<Props> = ({
  showAddModal,
  handleClose,
  selectedUser,
}) => {
  const [items, setItems] = useState<IUserLink[]>([]);
  ...
  const handleSave = async (selectedHomesIds: number[]) => {
    ...
    if (data.toUpdate.length > 0 || data.toDelete.length > 0) {
      ...
      setError(false);
      await api
        .put("/api/v1/home/links/users", data)
        .then(() => {
          toast({
            title: "Links Success",
            description: "Home Users linked successfully",
            duration: 5000,
            variant: "default",
          });
          handleClose();
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      await api
        .get("/api/v1/home/list/all-link/" + selectedUser.id)
        .then(({ data }) => {
          if (data.homes) {
            setItems(data.homes);
            const initiallySelected = data.homes
              .filter((home: IUserLink) => home.linked)
              .map((home: IUserLink) => home.id);
  ...
  return (
    <Dialog open={showAddModal} onOpenChange={handleClose}>
      <DialogContent
        ...
        aria-describedby={"link-home-users"}
      >
