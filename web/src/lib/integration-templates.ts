...
export const PROTOCOL_CATALOG: ProtocolInfo[] = [
  ...
  },
  {
    protocol: 'wifi',
    label: 'WiFi',
    image: 'ESPHome firmware',
    docsUrl: 'https://esphome.io/components/mqtt.html',
    description: 'WiFi devices with native MQTT firmware (ESPHome).',
  },
  {
    protocol: 'ble',
    label: 'BLE',
    ...
    case 'wifi':
      return {
        ...
        snippet: [
          ...
          `  username: ${p.username}`,
          `  password: ${p.password}`,
          `  client_id: ${clientId}-<device>   # unique per device`,
          `  topic_prefix: ${baseTopic}/<device>`,
          '  discovery: true',
