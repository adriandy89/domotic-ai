...
class SSEService {
    ...
    try {
      ...
      switch (message.topic) {
        ...
        case 'user.attributes.updated':
          this.handleUserAttributesUpdated(
            message.payload as UserAttributesUpdatedPayload,
          );
          break;
      ...
      const handler =
        this.customHandlers[message.topic as keyof SSEEventHandler];
      if (handler) {
  ...
  private handleSensorData(payload: SensorDataPayload): void {
    ...
    useDevicesStore
      .getState()
      .updateDeviceData(payload.deviceId, payload.data, payload.timestamp);
  }
  ...
  private async handleUserAttributesUpdated(
    payload: UserAttributesUpdatedPayload,
  ): Promise<void> {
